...
notifications <- dropdownMenu(type = "notifications", badgeStatus = "info",
                              notificationItem(
                                text = "GitHub",
                                icon("github-alt"),
                                href="https://github.com/TanmayKumar-EngStud"
                              )
)
...
server <- function(input, output ) {
  ...
  output$chart2_m<- DT::renderDataTable({
    ...
    stocks_new = c('SOXL','TECL','NAIL','TQQQ','FNGU','DFEN','USD','ROM','FAS','SPXL','UPRO','DUSL','UBOT','FNGO','WANT','QLD','UDOW','DRN','FBGX','NUGT') # More Stocks : 'FLGE','FRLG','UMDD','MIDU','UTSL','INR','KOLD','CHAU','CWEB','UXI','SMH','EURL','URTY','TNA','LMLP','XSD','SPUU','SSO','SOXX','LABU','TAN','UYG','CURE','FTXL','PBW','DZK','TPOR','WTIU','CNRG','OILU','PSI','URE','UGE','UWT','JNUG','URR','XLK','DPST','CHIK','DDM','FTEC','VGT','BBC','UCC','DGAZ','QTEC','IXN','CHIS','ACES','MVV','IYW','SVXY','PTF','QTUM','UCO','SNSR','CHIQ','PALL','UGLD','EDC','FIEE','UWM','JPNL','SMLL','FEUL','CNXT','GREK','QCLN','DWAQ','ROKT','UBIO','UPV','FFEU','FIYY'
  ...
  output$chart2_p<- DT::renderDataTable({
    ...
    stocks_new = c("LABU",	"AMZN",	"SOXL",	"TECL",	"UBER",	"AMD",	"MRVL",	"FB",	"NFLX",	"DIS",	"BYND","AXSM","KRMD",'EVER','KOD','RLMD','CDLX','ADVM','SAVA','APLT','RCEL','ENPH','STXS','ARWR','ZYME')
